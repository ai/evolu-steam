...
/**
 * Mark string as function, to use in `Runner#option`. Instead of usual string,
 * worker will eval it to real function.
 * 
 * You can put each string lines of your function as arguments, constructor will
 * join it to string:
 * 
 *   var f = evolu.steam.Function('function(a) {',
 *       'a += 1',
 *       'return a',
 *   '}')
 * 
 * @constructor
 */
evolu.steam.Function = function(string) {
...
evolu.steam.Function.prototype = {
    
    /**
     * Compile string representation of function to real function. Will be
    ...
    compile: function() {
        if (!this._compiled) {
            this._compiled = eval('(' + this.string + ')')
        }
        return this._compiled
    },
    
    /** Compile and call function. Just syntax sugar. */
    call: function() {
        this.compile()
        return this._compiled.apply(this, arguments)
    }
...
evolu.steam.Runner.prototype = {
     ...
     * 
     * WARNING: If you set function, don’t use any external variables
     * (closures). Function will be convert to string, send by JSON and next
     * compiled (evaled) in workers, because workers doesn’t has any shared
     * memory.
     *
    ...
    option: evolu.steam._afterInit('option',
        function(name, value) {
            var msg = { command: 'option', name: name }
            if ('function' == typeof(value)) {
                msg.func = value.toString()
            } else if (value instanceof evolu.steam.Function) {
                msg.func = value.string
            } else {
                msg.value = value
            }
    ...
    start: evolu.steam._afterInit('start',
        function() {
            return this.send({ command: 'start' })
        }),
